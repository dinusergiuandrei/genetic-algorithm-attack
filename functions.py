...
functions = [
    ...
    ('Schwefel 7', lambda x: np.sum(-x * np.sin(np.sqrt(np.abs(x)))), lambda n: - n * 418.9829, -500, 500),
    (
        'Griewangk 8',
        lambda x: np.sum(np.power(x, 2)) / 4000 - np.prod(np.cos(x / np.sqrt(np.arange(1, len(x) + 1)))) + 1,
        lambda n: - n * 418.9829, -600, 600),
